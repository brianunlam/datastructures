...
describe('testing binarySearchTree class with numbers', () => {
  let tree: BinarySearchTree<Nn>;
  beforeEach(() => {
    tree = new BinarySearchTree<Nn>((a: Nn, b: Nn) => a.n - b.n);
  });

  it('I should add the element to the empty tree', () => {
    tree.add({ n: 12 });
    expect(tree.length).toEqual(1);
  });

  it('I should add multiple elements to the tree', () => {
    const treeElements = [14, 8, 15, 10, 3];
    treeElements.forEach((n) => tree.add({ n }));
    expect(tree.length).toEqual(5);
  });

  it('I should add a repeating element to the tree', () => {
    const treeElements = [14, 8, 15, 10, 3];
    treeElements.forEach((n) => tree.add({ n }));
    expect(tree.add({ n: 8 })).toBe(false);
  });

  it('I should remove an element to the empty tree', () => {
    expect(tree.delete({ n: 12 })).toEqual(null);
  });

  it('I should remove an element to the tree', () => {
    const treeElements = [14, 8, 15, 10, 3];
    treeElements.forEach((n) => tree.add({ n }));
  ...
  it('I should remove an element to the tree', () => {
    const treeElements = [14, 8, 15, 10, 3];
    treeElements.forEach((n) => tree.add({ n }));
    expect(tree.delete({ n: 10 })).toEqual({ n: 10 });
  });

  it('I should remove an non-existent element to the tree', () => {
    const treeElements = [14, 8, 15, 10, 3];
    treeElements.forEach((n) => tree.add({ n }));
  ...
  it('it should return null in an empty tree', () => {
